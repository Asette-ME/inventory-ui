...
const Button = ({ children, onClick, disabled, variant = "primary", className = "", icon: Icon }) => {
  ...
  const variants = {
    ...
    magic: "bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white hover:from-violet-600 hover:to-fuchsia-600 shadow-sm hover:shadow-md border-0"
  };
...
export default function PaymentPlanExtractor() {
  const [file, setFile] = useState(null); // This will hold the converted image file
  const [originalPdfName, setOriginalPdfName] = useState(null);
  ...
  const renderPdfToImage = async (pdfFile) => {
      ...
      try {
          ...
          const page = await pdf.getPage(1); // Render first page only for this tool
          ...
          // Convert to Blob/File for consistency with Tesseract/Gemini
  ...
  const parseLineToRow = (line) => {
    const text = line.text.trim();
    if (!text || text.length < 4) return null;

    const percentPattern = /(\d+(\.\d+)?)\s?%/;
    const datePattern = /((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*[\s,.-]*\d{4})|(\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4})|On\s?Booking|Handover|Completion/i;
  ...
  const processImage = async () => {
    ...
    try {
      ...
      // Draw boundaries omitted for brevity in this specific function call logic, kept in drawBoundaries fn
      ...
      // Default to Heuristic Parsing initially
      const rows = result.data.lines
        .map(parseLineToRow)
        .filter(row => row !== null);

      if (rows.length === 0) {
        const fallbackRows = result.data.lines
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between mb-4">
                 ...
                 {imagePreview && !isProcessing && (
                   <Button variant="ghost" onClick={() => { setFile(null); setImagePreview(null); setOriginalPdfName(null); }} className="!p-1 text-slate-400 hover:text-red-500">
              ...
              {!imagePreview && !isPdfRendering ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className={`transition-all duration-300 ${isProcessing || isAiProcessing ? 'opacity-50 pointer-events-none' : ''}`}>
                    <button 
                    ...
                    {showContext && (
                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 space-y-3 mb-4 animate-in fade-in slide-in-from-top-2">
                            <div>
                                <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Total Property Value</label>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Booking Date</label>
                                    <input 
                                        type="date" 
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Handover Date</label>
                                    <input 
                                        type="date" 
                                        className="w-full px-2 py-1.5 text-sm border border-slate-300 rounded focus:ring-1 focus:ring-indigo-500 outline-none text-slate-600"
                                        value={handoverDate}
          ...
          <div className="lg:col-span-7 space-y-6">
             ...
             <Card className="flex flex-col min-h-[500px] h-auto">
                <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
                   ...
                   <div className="flex items-center gap-2">
                       <Button variant="ghost" onClick={handleAnalyzePlan} disabled={extractedData.length === 0 || isAiAnalyzing} className="!py-1 !px-3 text-xs text-violet-600 hover:bg-violet-50">
                   ...
                   {extractedData.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        ...
                        {extractedData.map((row) => (
                          <div key={row.id} className="grid grid-cols-12 gap-2 items-center bg-white p-2 rounded-lg border border-slate-200 shadow-sm hover:border-indigo-300 transition-colors group">
