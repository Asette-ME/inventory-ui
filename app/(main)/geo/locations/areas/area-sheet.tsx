...
import { Area, AreaCreateInput, AreaUpdateInput, Coordinates } from '@/types/entities';
...
interface AreaSheetProps {
  ...
  area: Area | null;
  onSave: (data: AreaCreateInput | AreaUpdateInput) => Promise<void>;
}

export function AreaSheet({ open, onOpenChange, area, onSave }: AreaSheetProps) {
  ...
  useEffect(() => {
    if (area) {
      ...
      if (area.coordinates) {
        ...
        setLatInput(area.coordinates.latitude.toString());
        setLngInput(area.coordinates.longitude.toString());
      } else {
        setCoordinates(null);
        setLatInput('');
        setLngInput('');
      }
      if (area.boundaries?.coordinates) {
        setBoundaries(area.boundaries.coordinates);
        setBoundariesInput(JSON.stringify(area.boundaries.coordinates, null, 2));
      } else {
        setBoundaries(null);
        setBoundariesInput('');
      }
      ...
      <SheetContent side="right" className="flex w-full flex-col gap-0 overflow-hidden p-0 sm:max-w-xl">
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <SheetHeader className="shrink-0 border-b p-6">
            <SheetTitle className="text-xl">{isEditing ? 'Edit Area' : 'Create Area'}</SheetTitle>
            <SheetDescription>{isEditing ? `Update ${area?.name}` : 'Add a new area to the system'}</SheetDescription>
          </SheetHeader>
