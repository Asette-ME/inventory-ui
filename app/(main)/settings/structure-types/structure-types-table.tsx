...
import { bulkDelete, deleteStructureType, getStructureTypes } from '@/lib/actions/entities';
import { cn } from '@/lib/utils';
import { PaginationMeta } from '@/types/common';
import { StructureType } from '@/types/entities';

import { StructureTypeSheet } from './structure-type-sheet';
import { DEFAULT_VISIBLE_COLUMNS, getStructureTypesColumns } from './structure-types-columns';
import { StructureTypesToolbar } from './structure-types-toolbar';

interface StructureTypesTableProps {
  sheetOpen: boolean;
  onSheetOpenChange: (open: boolean) => void;
}

export function StructureTypesTable({ sheetOpen, onSheetOpenChange }: StructureTypesTableProps) {
  const [items, setItems] = useState<StructureType[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<StructureType | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<StructureType | null>(null);
  ...
  const fetchItems = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await getStructureTypes({ search, page, limit, sort_by: sortBy, sort_order: sortOrder });
  ...
  function handleEdit(item: StructureType) {
    setSelectedItem(item);
    onSheetOpenChange(true);
  }

  function handleDelete(item: StructureType) {
  ...
  async function confirmDelete() {
    ...
    try {
      await deleteStructureType(itemToDelete.id);
      toast.success(`Structure type "${itemToDelete.name}" deleted successfully`);
      void fetchItems();
    } catch (err) {
  ...
  const columns = getStructureTypesColumns({
    sortBy,
    sortOrder,
    onSort: (key, order) => {
      setSortBy(key);
        ...
        actions={[
          {
            ...
            onClick: async () => {
              ...
              const results = await bulkDelete(deleteStructureType, ids);
              const succeeded = results.filter((r) => r.status === 'fulfilled').length;
              const failed = results.filter((r) => r.status === 'rejected').length;
              if (succeeded > 0) toast.success(`${succeeded} structure types deleted`);
